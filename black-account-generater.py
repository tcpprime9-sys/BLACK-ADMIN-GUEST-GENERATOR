...
BASE_FOLDER = os.path.join(CURRENT_DIR, "BLACK_ADMIN-ERA")
...
{client_data}
{Colors.RESET}
\n{get_random_color()}{Colors.BRIGHT}All accounts—including Rare, Couple, and Normal types—will be saved here in separate folders. In the event that a Rare ID is missed by the system, it will automatically be saved in the 'Normal Account' folder.
​Connection & IP Issues:
If your IP gets banned and you are not using Wi-Fi, please toggle Aeroplane Mode on and off. Alternatively, you may use a VPN to bypass the restriction.
​Troubleshooting Generation:
If someone is unable to generate an account, please try Option 11 (Ghost Mode). This uses all available servers. Once generated, the ID will be locked to the specific location/IP from which you first log in.
"""
    print(banner)
...
def generate_custom_password(prefix):
    ...
    return f"{prefix}_{random_part1}_BLACK_ADMIN_{random_part2}"
    ...
    while True:
        try:
            ...
            if choice == "00":
                ...
                print(f"\n{get_random_color()}{Colors.BRIGHT}👋 Thank you for using BLACK ADMIN Generator!{Colors.RESET}")
                sys.exit(0)
            elif choice.isdigit():
...
def about_section():
    ...
    print(f"{get_random_color()}{Colors.BRIGHT}ℹ️  About BLACK ADMIN Account Generator{Colors.RESET}")
...
def main_menu():
    while True:
        ...
        print(f"{get_random_color()}{Colors.BRIGHT}🎮 Welcome to BLACK ADMIN Account Generator{Colors.RESET}")
